...
void PrintLine(const string& text, bool prompt = true)
{
...
void RecvThread(SOCKET sock, atomic<bool>& running)
{
	while (running)
	{
		...
		case PKT_SC_CHAT:
		{
			...
			{
				PrintLine("[system] 잘못된 채팅 패킷을 수신했습니다.");
				break;
			}
		...
		case PKT_SC_KICK:
			...
			return;
		default:
			PrintLine("[system] 알 수 없는 패킷 수신 id=" + to_string(pktId) + ", bodySize=" + to_string(bodySize));
			break;
		}
	}
}

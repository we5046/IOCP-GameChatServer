...
| Unity 클라이언트 — 접속·대기실·준비·게임 시작 수신 | **완료·검증됨** (별도 저장소 `BombArenaClient`) |
| 서버 — 게임용 접속·방·준비 패킷 처리 | 미구현 |
| 서버 — 틱 루프, 이동·폭탄·판정 | 미구현 |

**현재 이 서버는 채팅 패킷(ID 1·2·3·4·1000·1001)만 처리합니다.**
게임용 패킷(10 이상)은 알 수 없는 ID로 판정되어 `SC_KICK` 후 연결이 끊깁니다.
따라서 Unity 클라이언트와의 실제 연동은 **아직 성립하지 않으며**, 클라이언트 측 검증은
프로토콜 v1을 구현한 별도 테스트 서버로 수행했습니다. 이 사실을 클라이언트 저장소에도 명시해 두었습니다.

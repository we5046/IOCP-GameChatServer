...
이 채팅 서버를 **서버 권위(server-authoritative) 방식의 실시간 2인 대전 게임 서버**로 확장하고 있습니다.
게임은 격자 맵에서 폭탄을 설치해 상대를 맞히는 「Bomb Arena」이며,
**이동·충돌·폭발·승패 판정을 전부 서버가 계산하고 클라이언트는 입력 전송과 화면 표시만 담당**합니다.

| 단계 | 상태 |
|---|---|
| 프로토콜 규약 확정 (`docs/protocol-v1.md`) | **완료** |
| Unity 클라이언트 — 접속·대기실·준비·게임 시작 수신 | **완료** (별도 저장소 `BombArenaClient`) |
| 서버 — 접속·방·준비·매치 시작 (10·11·20·21·22·30·1002) | **완료·연동 확인** |
| 서버 — 틱 루프, 이동·폭탄·판정 (31~34) | 미구현 |

**대기실 구간까지는 Unity 클라이언트와 실제로 연동됩니다.**
클라이언트가 `CS_HELLO`(10)를 보내면 서버가 버전을 확인하고 `SC_WELCOME`(11)으로 플레이어 ID를 부여하며,
`CS_JOIN_ROOM`(20) → `SC_ROOM_STATE`(21) → `CS_READY`(22) → 정원 전원 준비 시 `SC_GAME_START`(30)까지
서버가 상태를 관리합니다. 잘못된 요청은 연결을 끊지 않고 `SC_ERROR`(1002)로 사유를 돌려줍니다.

게임 진행 구간(31~34)은 고정 주기 틱 루프·충돌 판정·폭발 전파가 필요해 다음 단계로 분리했습니다.

### 대기실 구간 설계 요점

- **상태 전이는 검증 이후에만** — FSM 은 패킷을 받아들일 상태인지만 판단하고,
  버전·본문 길이·정원 검사는 처리 함수 안에서 수행합니다. 검사에 실패하면 상태를 바꾸지 않습니다.
- **거절과 종료를 구분** — 규약 위반(버전 불일치)은 연결 종료, 상황 거절(정원 초과·없는 방)은
  `SC_ERROR` 후 연결 유지. 클라이언트가 재시도할 수 있어야 하기 때문입니다.
- **접속 종료 시 잔여 인원 갱신** — `LeaveRoom` 은 플레이어의 방 포인터를 지우므로,
  방을 먼저 보관한 뒤 퇴장 처리하고 남은 인원에게 `SC_ROOM_STATE` 를 다시 보냅니다.
- **채팅 방과 게임 방 분리** — 게임 방은 `capacity` 를 가지며, 사용 중인 채팅 방(정원 0)은
  게임 방으로 전환하지 않고 `ERR_ROOM_FULL` 로 거절합니다.

### 사양을 먼저 확정한 이유

서버(C++)와 클라이언트(Unity/C#)를 **서로 다른 주체가 병렬로 개발**하므로,
양측이 직렬화 방식을 각자 추정하면 통합 시점에 전면 재작업이 발생합니다.
`docs/protocol-v1.md`는 모든 필드의 자료형·순서·길이·바이트 순서를 확정한 **유일한 접점 문서**입니다.

이 판단은 실제로 효과가 있었습니다. 클라이언트 구현 과정에서 **사양서 자체의 결함 5건**이
코드 작성 전에 드러났습니다. 그중 하나는 맵 생성 규칙이 두 플레이어의 시작 위치를
모두 벽으로 만드는 오류로, **서버 구현 전에 발견되어 잘못된 코드가 작성되지 않았습니다.**
정정 내역은 `docs/protocol-v1.md`의 「개정 1.1 상세」에 결함별 심각도와 함께 기록했습니다.
...
![방 격리 동작 확인](docs/images/room-isolation.png)

위 화면은 서버 1개와 클라이언트 3개를 동시에 실행한 결과입니다.

- 오른쪽 위가 **서버**입니다. 워커 스레드 12개가 생성되고, 세 클라이언트의 접속·로그인·방 입장이 순서대로 기록됩니다.
- 아래 두 개는 **Room 1** 의 `player1`, `player2` 입니다. 한쪽이 보낸 메시지가 다른 쪽에 도착합니다.
- 왼쪽 위는 **Room 2** 의 `player_alone` 입니다. 같은 서버에 접속해 있지만 **Room 1의 대화가 전달되지 않고**,
  이 클라이언트가 보낸 메시지도 Room 1로 새지 않습니다.

즉 브로드캐스트가 서버 전체가 아니라 **방 단위로 격리**되어 동작함을 확인할 수 있습니다.
...
└── docs/
    ├── images/                          실행 화면 캡처
    ├── protocol-v1.md                   Bomb Arena 통신 규약 (클라이언트와의 접점 문서)
...
위 6종은 **현재 구현된 채팅 패킷**입니다.
게임용 패킷 12종(ID 10·11·20·21·22·30~34·1002)을 포함한 전체 규약과
모든 필드의 바이트 단위 배치는 **[`docs/protocol-v1.md`](docs/protocol-v1.md)** 에 확정되어 있습니다.
해당 문서는 각 패킷에 구현 여부를 함께 표시하므로, 규약과 현재 구현 상태를 구분해 확인할 수 있습니다.
